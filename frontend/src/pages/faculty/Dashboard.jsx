...
const FacultyDashboard = () => {
      ...
      try {
        ...
        const pending = allSubmissions
          ...
          .map(s => ({
            ...
            course: s.classroom?.name || 'Unknown Course',
            submittedAt: new Date(s.submittedAt),
            version: `v${s.version || 1}`
          }));
        setPendingSubmissions(pending);

        // Filter recent reviews (reviewed in last 24 hours)
        const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
        const reviewed = allSubmissions
          .filter(s => s.reviewedAt && new Date(s.reviewedAt) > oneDayAgo)
          .map(s => ({
            id: s._id,
            title: s.assignment?.title || 'Untitled',
            studentName: s.student?.name || 'Unknown Student',
            reviewedAt: new Date(s.reviewedAt),
            status: s.status,
            grade: s.marks ? `${s.marks}/${s.assignment?.maxMarks || 100}` : null
          }));
        setRecentReviews(reviewed);
      } catch (error) {
  ...
  const submitFeedback = async (feedbackData) => {
    try {
      ...
      await apiService.reviewSubmission(feedbackData.submissionId, {
